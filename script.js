...
const playlists = [
    ...
    { name: "Global Pop", q: "Billboard Top 50", img: "https://images.unsplash.com/photo-1493225255756-d9584f8606e9?w=300" }
];

function initHome() {
    const grid = document.getElementById('home-grid');
    grid.innerHTML = playlists.map(p => `
        <div class="card" onclick="openPlaylist('${p.q}')" style="position: relative; border-radius: 15px; overflow: hidden; aspect-ratio: 1/1; cursor: pointer;">
            <img src="${p.img}" style="width: 100%; height: 100%; object-fit: cover; opacity: 0.6;">
            <div style="position: absolute; bottom: 10px; left: 10px; font-weight: bold; font-size: 16px; color: #fff;">${p.name}</div>
        </div>`).join('');
    updateLibUI();
}
    ...
    container.innerHTML = list.map((s, i) => {
        ...
        <div class="list-item" style="display: flex; align-items: center; gap: 12px; padding: 12px; border-bottom: 1px solid #111;">
            ...
            <div style="flex: 1; overflow: hidden;" onclick="playSong(${i})">
                <h4 style="font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #fff;">${s.snippet.title}</h4>
            </div>
...
function changeQuality(e) {
    ...
    alert("Quality: 720p (If available)");
    document.getElementById('mini-popup-menu').classList.add('hidden');
}
...
function showFavorites() {
    if(favorites.length === 0) return alert("Empty");
...
function showDownloads() {
    if(downloads.length === 0) return alert("Empty");

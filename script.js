...
const playlists = [
    { name: "Trending", q: "Latest Bollywood Songs 2025", img: "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=300" },
    { name: "Lofi Mix", q: "Hindi Lofi Chill Songs", img: "https://images.unsplash.com/photo-1516280440614-37939bbacd81?w=300" },
    { name: "Old is Gold", q: "Classic Kishore Kumar Hits", img: "https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?w=300" },
    { name: "90s Hits", q: "90s Romantic Bollywood Songs", img: "https://images.unsplash.com/photo-1493225255756-d9584f8606e9?w=300" },
    ...
    { name: "Summer", q: "Bollywood Summer Party Hits", img: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=300" }
];
...
// Click to Seek Function (Important)
...
async function openPlaylist(q) {
    ...
    // videoDuration: 'short' added to filter out long videos
    const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q=${encodeURIComponent(q)}&type=video&videoDuration=short&key=${API_KEY}`);
...
function renderSongList(list) {
    document.getElementById('song-list').innerHTML = list.map((s, i) => {
...
function toggleFav(id, idx) {
    ...
    updateLibUI();
    renderSongList(currentList); // Refresh heart colors
}
...
function showFavorites() {
    switchTab('home-section', document.querySelectorAll('.nav-item')[2]);
    document.getElementById('home-grid').classList.add('hidden');
    document.getElementById('playlist-view').classList.remove('hidden');
